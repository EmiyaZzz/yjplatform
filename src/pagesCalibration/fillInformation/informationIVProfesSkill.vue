<template>
  <informationWrapper :progressNum="progressNum">
    <view class="form-wrapper" slot="formWrapper">
      ...
      <caption-box title="职称"></caption-box>
      <view class="content-box at-row align-center pt-30 mb-30">
        <view class="single degrees" :class="[item.val === professionalTitleVal ? 'active' : '']" v-for="(item, index) in professionalTitle" :key="item.val" @click="changeProfessionalTitle(index)">
          <text>{{ item.name }}</text>
        </view>
      </view>
...
export default Vue.extend({
  ...
  onLoad () {
    getBaseInfoApi().then(res => {
      this.professionalTitleVal = res.professionalTitle
  ...
  methods: {
    updateBaseInfo () {
      ...
      const params = {
        ...
        rank: this.learnVal,
        professionalTitle: this.professionalTitleVal
      }
