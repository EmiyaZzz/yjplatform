          ...
          <view class="management-item mb-40" v-for="(item, index) in list" :key="item.id" :style="{'height':item.unfold ? '300rpx':'134rpx'}">
            <view class="reveal at-row align-center space-between">
              <view class="title at-row align-center">
                <view class="name ellipsis">{{ item.technologicalAchievementsName }}</view>
                <view class="grade" v-if="item.isNationalPatent==='0'">国家专利</view>
              </view>
            ...
            <view class="pb-50">
              <view v-if="item.technologicalAchievementMaterial.length>0">
                <view class="img-box" v-for="(val, tIndex) in item.technologicalAchievementMaterial" :key="tIndex">
                  <image :src="val" />
                </view>
              </view>
...
import { getTechnologicalQueryApi, delTechnologicalApi } from '@/api/common'
export default Vue.extend({
  ...
  methods: {
    getHonorQuery () {
      getTechnologicalQueryApi().then(res => {
        ...
        reslist.forEach(val => {
          val.unfold = false
          val.selected = false
          val.technologicalAchievementMaterial = val.technologicalAchievementMaterial ? JSON.parse(val.technologicalAchievementMaterial) : []
          this.list.push(val)
        })
      })
    },
    ...
    del () {
      ...
      delTechnologicalApi({ ids: ids.join(',') }).then(res => {
        this.getHonorQuery()
        this.$toast('删除成功')
      })
    },
    add () {
      this.$changePage('/pagesCalibration/fillInformation/addProfess')
    }
  }
})
</script>
