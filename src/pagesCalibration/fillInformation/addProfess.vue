  ...
  <rcyj-page-view>
    <view class="information-wrapper pb-70">
      <view class="topbg" :style="{'background-image':'url('+topBg+')'}">
        <view class="h1">添加技术成果</view>
      </view>
      <view class="form-wrapper add-form-wrapper">
        <caption-box title="技术名称"></caption-box>
        <view class="content-box line-box">
          <rcjy-input placeholder="填写技术名称" :inval="honorName" @input="honorChange"></rcjy-input>
        </view>
        <caption-box title="是否已申请国家专利" mb="mb-40">
        ...
        <view class="uploadTips mt-30 mb-70">上传的图片请保证字迹清晰、边角完整，大小5M以内，证件照片会保存在信息授权模块的身价证件包中。</view>
...
import { addTechnologicalApi } from '@/api/common'
export default Vue.extend({
  ...
  methods: {
    addHonor () {
      ...
      const params = {
        technologicalAchievementsName: this.honorName,
        isNationalPatent: this.scholarshipCheck ? '0' : '1', // 是0否1
        technologicalAchievementMaterial: this.honorCertificateMaterial.length ? JSON.stringify(this.honorCertificateMaterial) : ''
      }
      addTechnologicalApi(params).then(res => {
        this.$toast('添加成功')
        setTimeout(() => {
          this.$pageBack()
        }, 1500)
