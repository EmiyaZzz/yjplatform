          ...
          <view class="management-item mb-40" v-for="(item, index) in list" :key="item.id" :style="{'height':item.unfold ? '300rpx':'134rpx'}">
            <view class="reveal at-row align-center space-between">
              <view class="title at-row align-center">
                <view class="name ellipsis">{{ item.competitionName }}</view>
                <view class="grade">{{ item.competitionLevel | statusFilter }}</view>
              </view>
            ...
            <view class="pb-50">
              <!-- <view class="prize-title pt-20 pb-40">{{ item.prizeTit }}：{{ item.prizeName }}</view> -->
              <view v-if="item.competitionAchievementMaterial.length>0">
                <view class="img-box" v-for="(val, tIndex) in item.competitionAchievementMaterial" :key="tIndex">
                  <image :src="val" />
                </view>
              </view>
...
import { getCompetitionQueryApi, delCompetitionApi } from '@/api/common'
export default Vue.extend({
  ...
  methods: {
    getCompetitionQuery () {
      getCompetitionQueryApi().then(res => {
        ...
        reslist.forEach(val => {
          val.unfold = false
          val.selected = false
          val.competitionAchievementMaterial = val.competitionAchievementMaterial ? JSON.parse(val.competitionAchievementMaterial) : []
          this.list.push(val)
        })
      })
    },
    ...
    del () {
      ...
      delCompetitionApi({ ids: ids.join(',') }).then(() => {
        this.getCompetitionQuery()
        this.$toast('删除成功')
      })
    },
    add () {
      this.$changePage('/pagesCalibration/fillInformation/addCompetition')
    }
  }
})
</script>
