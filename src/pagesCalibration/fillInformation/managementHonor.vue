          ...
          <view class="management-item mb-40" v-for="(item, index) in list" :key="item.id" :style="{'height':item.unfold ? '300rpx':'134rpx'}">
            <view class="reveal at-row align-center space-between">
              <view class="title at-row align-center">
                <view class="name ellipsis">{{ item.honorName }}</view>
                <view class="grade">{{ item.honorLevel | statusFilter }}</view>
              </view>
            ...
            <view class="pb-50">
              <view v-if="item.honorCertificateMaterial.length>0">
                <view class="img-box" v-for="(val, tIndex) in item.honorCertificateMaterial" :key="tIndex">
                  <image :src="val" />
                </view>
              </view>
...
import { getHonorQueryApi, delHonorApi } from '@/api/common'
export default Vue.extend({
  ...
  methods: {
    getHonorQuery () {
      getHonorQueryApi().then(res => {
        ...
        reslist.forEach(val => {
          val.unfold = false
          val.selected = false
          val.honorCertificateMaterial = val.honorCertificateMaterial ? JSON.parse(val.honorCertificateMaterial) : ''
          this.list.push(val)
        })
      })
    },
    ...
    del () {
      ...
      delHonorApi({ ids: ids.join(',') }).then(res => {
        this.getHonorQuery()
        this.$toast('删除成功')
      })
    },
    add () {
      this.$changePage('/pagesCalibration/fillInformation/addHonor')
    }
  }
})
</script>
