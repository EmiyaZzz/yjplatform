  ...
  <rcyj-page-view>
    <view class="information-wrapper pb-70">
      <view class="topbg" :style="{'background-image':'url('+topBg+')'}">
        <view class="h1">添加科研成果</view>
      </view>
      <view class="form-wrapper add-form-wrapper">
        <caption-box title="科研项目名称"></caption-box>
        <view class="content-box line-box">
          <rcjy-input placeholder="填写科研项目名称" :inval="honorName" @input="honorChange"></rcjy-input>
        </view>
        <caption-box title="项目等级"></caption-box>
...
import { addScientificApi } from '@/api/common'
export default Vue.extend({
  ...
  methods: {
    addHonor () {
      ...
      const params = {
        scientificAchievementsName: this.honorName,
        scientificAchievementLevel: this.gradeVal,
      ...
      addScientificApi(params).then(res => {
        this.$toast('添加成功')
        setTimeout(() => {
          this.$pageBack()
        }, 1500)
